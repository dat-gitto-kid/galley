...
if [ "$ROM" == true ]; then
  ...
  do
    ...
    if [[ "$ROOT_TYPE" == "magisk" ]]; then
      if [ "$target" == "shiba" ] || [ "$target" == "oriole" ]; then
          echo -e "${RED}Shiba detected ${NC}"
          export preinit=metadata
      elif [[ "$target" == "lynx" ]]; then
          echo -e "${RED}Lynx detected ${NC}"
          export preinit=persist
